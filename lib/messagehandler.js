...
function Send(command, data) {
	//console.log("(Master): Posting message: ", command, data);
    worker.postMessage(
    {
        "command" : command,
        "data" : data
...
function OnMessage(e) {
	//console.log("(MASTER) messagehandler: OnMessage", e);
...
function SetWorker(_worker) {
    ...
    Register("Abort", function(){Debug("Master: Received abort signal from worker"); run=false; process.exit(0);});
